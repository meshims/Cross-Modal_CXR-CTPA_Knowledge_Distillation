...
from dalle2_local.loss.classification_loss import classification_loss
...
def combined_mse_classifier_loss(prediction, target):

    l2_loss = F.mse_loss(prediction, target)

    computed_classification_loss= classification_loss(prediction, target)

    return 0.8*l2_loss + 0.2*computed_classification_loss
...
class NoiseScheduler(nn.Module):
    def __init__(self, *, beta_schedule, timesteps, loss_type, p2_loss_weight_gamma=0., p2_loss_weight_k=1):
        ...
        if loss_type == 'l1':
            ...
            loss_fn = combined_mse_classifier_loss
...
class DiffusionPrior(nn.Module):
    ...
    def p_losses(self, image_embed, times, text_cond, noise=None, labels = None):
        ...
        if self.loss_type =="custom":
            loss = self.noise_scheduler.loss_fn(pred, target, labels)
        else:
    ...
    def forward(
        ...
        return self.p_losses(image_embed, times, text_cond=text_cond, labels = labels, *args, **kwargs)
