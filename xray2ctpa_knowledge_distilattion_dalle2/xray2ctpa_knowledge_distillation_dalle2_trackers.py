...
from .xray2ctpa_knowledge_distillation_dalle2_pytorch import Decoder, DiffusionPrior
from .xray2ctpa_knowledge_distillation_dalle2_trainer import DecoderTrainer, DiffusionPriorTrainer
...
class BaseLogger:
    ...
    def __init__(self, data_path: str, resume: bool = False, auto_resume: bool = False, verbose: bool = False,
                 **kwargs):
...
class LocalLoader(BaseLoader):
    ...
    def recall(self) -> dict:
        # Load the file
        print("loading from path " + str(self.file_path))
...
class HuggingfaceSaver(BaseSaver):
    ...
    def init(self, logger: BaseLogger, **kwargs):
        ...
        self.hub = import_or_print_error('huggingface_hub',
                                         '`pip install huggingface_hub` to use the huggingface saver')
        try:
            ...
            if not os.path.exists(self.token_path):
                raise Exception(
                    "Not logged in to huggingface and no token_path specified. Please login with `huggingface-cli login` or if that does not work set the token_path.")
...
class Tracker:
    def __init__(self, data_path: Optional[str] = DEFAULT_DATA_PATH, overwrite_data_path: bool = False,
    ...
    def _load_auto_resume(self) -> bool:
        ...
        if auto_resume_dict["logger_type"] != self.logger.__class__.__name__:
            raise Exception(
                f'The logger type in the auto_resume file is {auto_resume_dict["logger_type"]} but the current logger is {self.logger.__class__.__name__}. Either use the original logger type, set `auto_resume` to `False`, or delete your existing tracker-data folder.')
    ...
    def init(self, full_config: BaseModel, extra_config: dict):
        ...
        self.blacklisted_checkpoint_metadata_keys = ['scaler', 'optimizer', 'model', 'version', 'step',
                                                     'steps']  # These keys would cause us to error if we try to save them as metadata
    ...
    def _save_state_dict(self, trainer: Union[DiffusionPriorTrainer, DecoderTrainer], save_type: str, file_path: str,
                         **kwargs) -> Path:
        """
        ...
        if save_type == 'checkpoint':
            ...
            metadata = {k: v for k, v in self.save_metadata.items() if
                        k not in self.blacklisted_checkpoint_metadata_keys}
            ...
            if isinstance(trainer, DiffusionPriorTrainer):
                ...
                raise NotImplementedError(
                    'Saving this type of model with EMA mode enabled is not yet implemented. Actually, how did you get here?')
            state_dict = {
                **self.save_metadata,
    ...
    def save(self, trainer, is_best: bool, is_latest: bool, save_only_latest=False, **kwargs):
        if self.dummy_mode:
            return
